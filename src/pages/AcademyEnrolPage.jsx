...
export default function AcademyEnrolPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const modeLabel = form.mode === "online" ? "Online (Self-Paced)" : "In-Person (Cape Town)";

    try {
      // Admin notification — goes to info@camluk.co.za (hardcoded in template dashboard)
      await emailjs.send(
        ...
        import.meta.env.VITE_EMAILJS_TEMPLATE_ID,
        {
          ...
          subject:    `New Academy Enrolment — ${selectedCourse?.label}`,
          message:    `New academy enrolment received.\n\nStudent: ${form.name}\nEmail: ${form.email}\nPhone: ${form.phone || "Not provided"}\n\nCourse: ${selectedCourse?.label}\nPrice: ${selectedCourse?.price}\nMode: ${modeLabel}\n\nAdditional notes:\n${form.message || "None"}`,
        },
        import.meta.env.VITE_EMAILJS_PUBLIC_KEY
      );

      // Student confirmation — goes to the student's email (To Email in dashboard = {{from_email}})
      await emailjs.send(
        import.meta.env.VITE_EMAILJS_SERVICE_ID,
        import.meta.env.VITE_EMAILJS_ENROL_TEMPLATE_ID,
        {
          from_name:    form.name,
          from_email:   form.email,
          course_name:  selectedCourse?.label,
          course_price: selectedCourse?.price,
          course_mode:  modeLabel,
        },
        import.meta.env.VITE_EMAILJS_PUBLIC_KEY
      );
