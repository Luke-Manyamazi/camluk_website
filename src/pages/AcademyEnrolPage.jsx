...
export default function AcademyEnrolPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const templateParams = {
      ...
      message: `Course: ${selectedCourse?.label} (${selectedCourse?.price})\nMode: ${modeLabel}\nPhone: ${form.phone || "Not provided"}\n\nAdditional notes:\n${form.message || "None"}`,
      to_email: "info@camluk.co.za",
    };

    try {
      // Admin notification → info@camluk.co.za
      await emailjs.send(
      ...
      // Student confirmation → from_email (uses separate template)
      if (import.meta.env.VITE_EMAILJS_ENROL_TEMPLATE_ID) {
        await emailjs.send(
          import.meta.env.VITE_EMAILJS_SERVICE_ID,
