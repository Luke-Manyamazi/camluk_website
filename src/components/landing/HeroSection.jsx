...
export default function HeroSection({ onNavigate }) {
  ...
  const handleCTA = (action) => {
    if (onNavigate && ["contact", "services", "process", "why-us"].includes(action)) { onNavigate(action); return; }
  ...
  return (
    <section id="home" className="relative min-h-screen flex flex-col overflow-hidden" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      {slides.map((s, i) => <div key={i} className="absolute inset-0 transition-opacity duration-1000" style={{ opacity: i === current ? 1 : 0 }}><img src={s.image} alt={s.alt} className="w-full h-full object-cover" style={{ objectPosition: s.position }} loading={i === 0 ? "eager" : "lazy"} /><div className="absolute inset-0 bg-gradient-to-r from-background/95 via-background/70 to-background/30" /><div className="absolute inset-0 bg-gradient-to-t from-background/80 via-transparent to-transparent" /></div>)}
      <div className="relative z-10 flex flex-col justify-end flex-1 max-w-6xl mx-auto w-full px-6 lg:px-10 pb-28 pt-32">
        <div key={`eyebrow-${current}`} className="inline-flex items-center gap-2 mb-6 w-fit px-3 py-1.5 border border-primary/40 bg-primary/10 backdrop-blur-sm text-primary animate-fade-in"><span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" /><span className="text-[10px] sm:text-xs font-mono font-semibold uppercase tracking-[0.22em]">{slide.eyebrow}</span></div>
        ...
        <div key={`cta-${current}`} className="flex flex-wrap gap-4 animate-fade-in"><button onClick={() => handleCTA(slide.ctas[0].action)} className="group inline-flex items-center gap-2 bg-primary text-primary-foreground font-bold px-8 py-4 text-base hover:bg-primary/90 hover:gap-3 transition-all">{slide.ctas[0].label}<ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" /></button><button onClick={() => handleCTA(slide.ctas[1].action)} className="inline-flex items-center gap-2 border-2 border-border text-foreground font-semibold px-8 py-4 text-base hover:border-primary hover:text-primary transition-all">{slide.ctas[1].label}</button></div>
