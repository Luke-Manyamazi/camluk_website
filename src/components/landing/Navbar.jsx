      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          <motion.div className="flex items-center gap-2.5 cursor-pointer" whileHover={{ scale: 1.02 }} onClick={() => handleNavClick({ sectionId: "home" })}>
            <div className="w-9 h-9 rounded-lg overflow-hidden"><img src={logo} alt="CL Camluk Technologies logo" className="w-full h-full object-cover" /></div>
            <span className="text-base sm:text-lg font-bold tracking-tight text-foreground">Camluk Technologies</span>
          </motion.div>
        ...
        {mobileOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-[#0a0a0a]/95 backdrop-blur-md">
            ...
            <div className="flex h-full w-full items-center justify-center px-6"><div className="space-y-4 text-center">
              {navLinks.map((link, index) => <motion.button key={link.sectionId} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.06 }} onClick={() => handleNavClick(link)} className="block w-full text-2xl sm:text-4xl font-black uppercase tracking-[-0.05em] text-white/80 hover:text-white transition-colors">{link.label}</motion.button>)}
            </div></div>
          </motion.div>
        )}
      </AnimatePresence>
